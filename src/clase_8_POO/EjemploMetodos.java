...
import java.util.Scanner;
        //EJEMPLO TOMADO DE SOLUCION DEL EXAMEN 1 (CLASE 7)
public class EjemploMetodos {
    public static void main(String[] args) {
        ...
        String textoLimpio = removerEspacios(cadenas[0]);
        int[] enteroCadenas0 = convertirStringConComasAVector(textoLimpio);

        textoLimpio = removerEspacios(cadenas[1]);
        int[] enteroCadenas1 = convertirStringConComasAVector(textoLimpio);
    ...
    public static int[] convertirStringConComasAVector(String textoSeparadoPorComas){
        String [] aux = textoSeparadoPorComas.replace(" ","").split(",");
        int[] enteroCadenas1 = new int[aux.length];
        for (int i = 0; i < aux.length; i++) {
            enteroCadenas1[i] = Integer.parseInt(aux[i]);
        }
        return enteroCadenas1;
    }
}